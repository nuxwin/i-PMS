...
class Forums_ThreadsController extends Zend_Controller_Action
{
	...
	public function addAction()
	{
		...
		if($request->isPost() && $form->isValid($request->getParam('threadForm'))) {
			try {
				...
				throw new Zend_Controller_Action_Exception(sprintf(
					'Unable to create new threa: ', $e->getMessage()), 500, $e);
			}

			$this->_redirect($this->urlHelper->url(array('tid' => $tid), 'forums_threads_show'));
		}
	...
	public function replyAction()
	{
		...
		if($request->isPost() && $form->isValid($request->getParam('replyForm'))) {
			...
			$this->_redirect($this->urlHelper->url(array('tid' => $tid), 'forums_threads_show') . "#pid{$pid}");
		}
		...
		$form->setAction($this->urlHelper->url(array('tid' => $tid), 'forums_posts_reply'));
	...
	public function deleteThreadAction()
	{
		...
		echo $this->urlHelper->url(array(), 'forums_add');
	...
	public function deletePostAction()
	{
		...
		// Get post info
		$modelPosts = new Forums_Model_DbTable_Posts();
		...
		// TODO get these data from a cache
		$threadModel = new Forums_Model_DbTable_Threads();
